...
function Projects() {
  ...
  const renderProjects = (type) => {
    ...
    return displayedProjects.map((project, index) => (
      <div key={index} className="w-36 bg-[#aaaaaa29] p-[0.33rem] rounded-md">
        <h2 className="text-white text-[0.7rem] font-semibold">
          {project.name}
        </h2>
        <p className="text-neutral-500 text-[0.5rem] text-justify">
          {project.para}
        </p>
        <a
          href={project.link}
          className="text-white text-[0.6rem] flex items-center gap-1"
        >
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center py-8 gap-8 relative">
